...
def load_advert(path, show=False, **kwargs):
   ...
   FUNCTION:
     ...
     INPUTS:
        ...
        kwargs{"size"} =BFSelectBased output size
    OUTPUTS:
        Advert image
    """
#'-----------------------------------------------------------------------------#
        ...
        for y in range(0, image.shape[0] - vstride, vstride):
            for x in range(0, image.shape[1] - hstride, hstride):
                ...
                if show:
                    ...
                    cv2.putText(ff, "TL and TR : {}".format((y, x)), cent - 15, cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                                (255, 255, 255), 2)
                    cv2.putText(ff, "Dist : {}".format((int(dist))), cent, cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                                (255, 255, 255), 2)
...
image1 = "/home/gaurab/PycharmProjects/markerless_ar-master/input_advert/advert.jpg"
image2 = "/home/gaurab/PycharmProjects/markerless_ar-master/image1.jpg"
...
def match_frames(image1, image2, method='BFMBased', **kwargs):
   ...
   FUNCTION:
    ...
    if method == 'FlannBased':
        ...
        # grayframe =   # trainimage
        # kp_grayframe, desc_grayframe = sift.detectAndCompute(grayframe, None)
        kp2, des2 = orb.detectAndCompute(image2, None)
        FLANN_INDEX_LSH = 6
        index_params = dict(algorithm=FLANN_INDEX_LSH, table_number=6, key_size=10, multi_probe_level=3)  # 6,10,3
